...
public class OrderController extends BaseController
{
     ...
     * 取消订单
     */
    @PreAuthorize("@ss.hasPermi('manage:order:cancel')")
    @Log(title = "订单管理", businessType = BusinessType.UPDATE)
     ...
     * 申请退款
     */
    @PreAuthorize("@ss.hasPermi('manage:order:refund')")
    @Log(title = "订单管理", businessType = BusinessType.UPDATE)
    @PostMapping("/refund/{id}")
    public AjaxResult applyRefund(@PathVariable Long id) {
        return toAjax(orderService.applyRefund(id));
    }

    /**
     * 撤销退款
     */
    @PreAuthorize("@ss.hasPermi('manage:order:cancelRefund')")
    @Log(title = "订单管理", businessType = BusinessType.UPDATE)
    @PostMapping("/cancel-refund/{id}")
    public AjaxResult cancelRefund(@PathVariable Long id) {
        return toAjax(orderService.cancelRefund(id));
    }
}